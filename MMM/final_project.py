...
def draw_poses(img, poses, point_score_threshold, skeleton=default_skeleton):
    ...
    return img
MAXCNT = 60
def detect_pose(img, poses, point_score_threshold, skeleton=default_skeleton)->int:
    if poses.size == 0:
        return 0
    global cnt
    img_limbs = np.copy(img)
    ...
    for pose in poses:
        ...
        if points[9,1] < points[0,1] and  points[10,1] < points[0,1] and points[9,0] > points[7,0] and points[10,0] < points[8,0]:
            ...
            cv2.putText(img,str(points[8,:]),points[8,:],font, font_scale, (255, 0, 0), thickness)
            if cnt == MAXCNT : 
        ...
        if points[9,1] < points[0,1] and  points[10,1] < points[0,1] and points[9,0] < points[7,0] and points[10,0] > points[8,0]:
            ...
            cv2.putText(img,str(points[8,:]),points[8,:],font, font_scale, (0, 255, 0), thickness)
...
def do_composition(img, bgimg):
    # Background removal
    # compostion
    # return
    print("파일이 존재하여 함수가 호출되었습니다.")
    IMAGE_URI = img
    image = cv2.cvtColor(
...
def run_pose_estimation(flip=False, use_popup=False, skip_first_frames=0):
    ...
    while True:
        ...
        detected_pose = detect_pose(frame, poses, 0.1)
